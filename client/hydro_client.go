...
func (client *HydroClient) getHydroSignature() string {
	hydroAuthStr := fmt.Sprintf("ZEEDEX-AUTHENTICATION@%d", time.Now().UnixNano()/int64(time.Millisecond))
...
func (client *HydroClient) get(path string, params []utils.KeyPair) (string, error) {
	...
	return utils.Get(
		...
		params,
		[]utils.KeyPair{
			{"Zeedex-Authentication", hydroAuthStr},
...
func (client *HydroClient) CreateOrder(
	...
	if validAmount.LessThan(client.minAmount) {
		return "", errors.New(fmt.Sprintf("Zeedex client %s create order amount %s less than min amount %s", client.TradingPair(), validAmount.String(), client.minAmount.String()))
	}
	...
	if placeSuccess {
		logrus.Infof("Zeedex client %s create order - price:%s amount:%s side:%s %s", client.TradingPair(), validPrice, validAmount, side, orderId)
		return orderId, nil
	} else {
		return "", errors.New(fmt.Sprintf("Zeedex client %s place order failed", client.TradingPair()))
	}
}
...
func (client *HydroClient) CancelOrder(orderId string) error {
	...
	_ = json.Unmarshal([]byte(resp), &dataContainer)
	if dataContainer.Desc != "success" {
		return errors.New(fmt.Sprintf("Zeedex client %s cancel order %s failed", client.TradingPair(), orderId))
	} else {
		logrus.Infof("Zeedex client %s cancel order %s succeed", client.TradingPair(), orderId)
		return nil
	}
}
...
func (client *HydroClient) GetOrder(orderId string) (StdOrder, error) {
	...
	_ = json.Unmarshal([]byte(resp), &dataContainer)
	if dataContainer.Desc != "success" {
		return orderData, errors.New(fmt.Sprintf("Zeedex client %s get order failed", client.TradingPair()))
	} else {
...
func (client *HydroClient) GetAllPendingOrders() ([]StdOrder, error) {
	...
	for true {
		...
		_ = json.Unmarshal([]byte(resp), &dataContainer)
		if dataContainer.Desc != "success" {
			return allOrders, errors.New(fmt.Sprintf("Zeedex client %s get all pending orders failed", client.TradingPair()))
		}
